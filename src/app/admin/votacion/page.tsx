...
export default function ResultadosVotacionPage() {
  ...
  useEffect(() => {
    async function cargarResultados(mostrarCarga = false) {
      ...
      const { data: votaciones, error: votacionesError } = await supabase
        .from("votaciones")
        .select("id, abierta")
        .eq("edicion_id", 1);

      if (votacionesError) {
        setMensaje(`Error votación: ${votacionesError.message}`);
        setCargando(false);
        return;
      }
  ...
  const missResuelta =
    !votacionAbierta && categoriaResuelta(miss);

  const misterResuelto =
    !votacionAbierta && categoriaResuelta(mister);
  ...
  async function guardarPuntos() {
    ...
    if (votacionAbierta) {
      setMensaje(
        "Primero cerrá la votación antes de guardar los puntos."
      );
      return;
    }
    ...
    if (!puedeCalcular) {
      setMensaje(
        "Todavía hay empates sin resolver."
      );
      return;
    }
  ...
  function renderDesempates(
    ...
    if (votacionAbierta) return null;
            ...
            >
              ...
              {!resuelto && (
                <p className="mt-3 text-xs font-semibold text-amber-700">
                  Cada candidato empatado debe tener un orden diferente.
                </p>
              )}
            </div>
          );
  ...
  return (
    <main className="min-h-screen bg-[#f7f7f5] px-5 py-8 text-zinc-900">
      <div className="mx-auto max-w-4xl">
        ...
        >
          {votacionAbierta
            ? "🟢 Votación abierta · resultados provisionales"
            : "🔴 Votación cerrada · resultado final"}
        </div>

        <p className="mt-3 text-sm text-zinc-500">
          Cada categoría otorga 300 / 250 / 200 / 150 puntos.
        </p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-semibold text-zinc-400">
                            {votacionAbierta
                              ? "Disponible al cerrar la votación"
                              : puedeCalcular
